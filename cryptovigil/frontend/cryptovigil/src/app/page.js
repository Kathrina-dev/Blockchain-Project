    ...
    if (token) {
      try {
        ...
        if (response.ok && data.user) {
          ...
          setUser(null);
          if (data.msg === "Invalid token" || data.msg === "Token expired") {
            localStorage.removeItem("token");
    ...
    <img src="/github.png" alt="GitHub" className="h-6 w-6" />
      ...
      <div className="fixed bottom-6 right-6 z-50">
        ...
        <button
          ...
          className="bg-[#63D134] text-white p-3 rounded-full shadow-lg hover:bg-green-600 transition-colors"
        >
          <svg
        ...
        {/* Chatbot component */}
        {isChatbotOpen && (
          <div className="mt-4 w-80 h-96 bg-white rounded-xl shadow-xl overflow-hidden">
            <Chatbot />
          </div>
        )}
      </div>
